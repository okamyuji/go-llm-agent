...
type Terminal struct {
	...
	// search holds the reverse incremental search state (search.go).
	search searchState
}
...
func bytesToKey(b []byte, pasteActive bool) (rune, []byte) {
	...
	if !pasteActive {
		...
		case 16: // ^P
			return keyUp, b[1:]
		case 18: // ^R
			return keyReverseSearch, b[1:]
		case 7: // ^G
			return keyAbortSearch, b[1:]
		}
	}
...
func (t *Terminal) moveCursorToPos(pos int) {
	...
	x, y := cellPos(t.displayPrompt(), t.line[:pos], t.termWidth)
...
func (t *Terminal) setLine(newLine []rune, newPos int) {
	...
	// moveCursorToPos は t.line を参照するため、新しい行を確定してから呼ぶ
	t.line = newLine
	t.pos = newPos
	if echo {
